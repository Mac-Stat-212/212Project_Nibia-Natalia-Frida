...
```{r warning = FALSE}
example_states <- c("Amazonas", "Pará", "Mato Grosso", "Rondônia")
...
annual_tc_loss <- function(state){
  ...
  ggplot(filter(state_loss, subnational1 %in% state), aes(x = year, y = total_loss)) +
    geom_line(color = "Annual Loss") +
    geom_line(aes(y = roll_avg_3yr, color = "3-Year Rolling Avg"), size = 1) +
    geom_smooth(aes(color = "Linear Trend"), method = "lm", se = FALSE, linetype = "dashed", linewidth = 0.5) +
    ...
    scale_color_manual(name = "Line Type",
                      values = c("Annual Loss" = "gray70",
                                 "3-Year Rolling Avg" = "darkgreen",
                                 "Linear Trend" = "red"))+
